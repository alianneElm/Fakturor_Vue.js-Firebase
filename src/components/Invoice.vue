<template>
  <router-link class="invoice flex" :to="{ name: 'Invoice', params: { invoiceId: invoice.invoiceId } }">
    ...
    <div class="right flex">
      <span class="price">{{ invoice.invoiceTotal }} kr</span>
      <div
        class="status-button flex"
        :class="{ paid: invoice.invoicePaid, draft: invoice.invoiceDraft, pending: invoice.invoicePending }"
      >
        <span v-if="invoice.invoicePaid">Betald</span>
        <span v-if="invoice.invoiceDraft">Utkast</span>
        <span v-if="invoice.invoicePending">Utestående</span>
      </div>
      <div class="icon">
